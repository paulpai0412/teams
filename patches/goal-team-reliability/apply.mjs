...
try {
  ...
  console.error(
    `goal-team-reliability: Cannot read patch manifest: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exit(1);
}
...
function packageVersion(target) {
  ...
  for (const item of manifest.requiredFiles ?? []) {
    safeRelative(item.path, "prerequisite path");
    if (hashFile(path.join(target, item.path)) !== item.sha256)
      fail(`Required base patch hash mismatch: ${item.path}`);
  }
...
try {
  ...
  console.error(
    `goal-team-reliability: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exitCode = 1;
}
