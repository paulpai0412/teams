...
pi.registerTool(defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		if (params.status === "complete") {
			const evidenceKey = goalEvidenceKey(core.state.goal);
			const teamBlock = await goalTeamRunBlockReason(ctx.sessionManager.getBranch(), core.state.goal.id, ctx.cwd, params.task_id);
			core.reconcileFocusedGoalFromDisk(ctx);
			if (!core.isFocusedOperationCurrent(taskFocus)) return core.focusedOperationCancelledResult("Task completion", taskFocus);
			if (core.state.goal?.status !== "active") {
				return { content: [{ type: "text", text: "Task completion cancelled because the goal is no longer active." }], details: goalDetails(core.state.goal) };
			}
			if (teamBlock) return { content: [{ type: "text", text: teamBlock }], details: goalDetails(core.state.goal) };
			const settings = loadGoalSettings(ctx.cwd);
			if (!settings.disableContracts && goalEvidenceKey(core.state.goal) !== evidenceKey) return { content: [{ type: "text", text: "Team evidence: task intent/contract changed; reconcile before retrying." }], details: goalDetails(core.state.goal) };
			const evidence = params.evidence?.trim().slice(0, 200) || undefined;
			if (evidenceKey !== null && !settings.disableContracts) {
				try {
					core.goalService.endTurn(ctx);
					if (core.goalService.isTurnBuffered()) throw new Error("Pending Goal state is still locked.");
				} catch (error) {
					return { content: [{ type: "text", text: `Task completion was not applied: ${error instanceof Error ? error.message : String(error)}` }], details: goalDetails(core.state.goal) };
				}
			}
			const result = core.goalService.updateTask(ctx, {
				focusToken: taskFocus,
				...
				validate: (task) => {
					...
					const evidenceBlock = settings.disableContracts ? null : goalEvidenceBlockReason({ id: core.state.goal!.id, taskList: { tasks: [task] } }, ctx.cwd, params.task_id);
					if (evidenceBlock) return { ok: false, message: evidenceBlock };
					return { ok: true };
				},
