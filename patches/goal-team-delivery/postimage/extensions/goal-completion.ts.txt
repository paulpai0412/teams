...
import { goalTeamRunBlockReason } from "./goal-team-hold.ts";
import { goalEvidenceBlockReason, goalEvidenceKey } from "./goal-team-evidence.mjs";
...
export async function runGoalCompletionFlow(core: GoalCore, ctx: ExtensionContext, completionSummary?: string): Promise<AgentToolResult<unknown>> {
	...
	const evidenceKey = goalEvidenceKey(auditTarget);
	function evidenceBlockNow(): string | null {
		const currentSettings = loadGoalSettings(ctx.cwd);
		if (currentSettings.disableTasks || currentSettings.disableContracts) return null;
		if (evidenceKey === null && goalEvidenceKey(core.state.goal) === null) return null;
		core.reconcileFocusedGoalFromDisk(ctx);
		const current = core.state.goal ? mergeGoalPromptFromDisk(ctx, core.state.goal) : null;
		if (!core.isFocusedOperationCurrent(completionFocus) || goalEvidenceKey(current) !== evidenceKey) return "Team evidence: Goal intent/contract changed during completion; reconcile before retrying.";
		return goalEvidenceBlockReason(current, ctx.cwd);
	}
	const teamBlock = await goalTeamRunBlockReason(ctx.sessionManager.getBranch(), auditTarget.id, ctx.cwd);
	if (!core.isFocusedOperationCurrent(completionFocus)) return core.focusedOperationCancelledResult("Goal completion", completionFocus);
	if (teamBlock) return { content: [{ type: "text", text: teamBlock }], details: goalDetails(core.state.goal) };
	const evidenceBlock = evidenceBlockNow();
	if (evidenceBlock) return { content: [{ type: "text", text: evidenceBlock }], details: goalDetails(core.state.goal) };
...
function commitGoalCompletion(core: GoalCore, ctx: ExtensionContext, opts: {
	...
	const evidenceBlock = evidenceBlockNow();
	if (evidenceBlock) return { ok: false, content: [{ type: "text", text: evidenceBlock }], details: goalDetails(core.state.goal), terminate: false };
	...
	try {
		const currentSettings = loadGoalSettings(ctx.cwd);
		if (evidenceKey !== null && !currentSettings.disableTasks && !currentSettings.disableContracts) {
			core.goalService.endTurn(ctx);
			if (core.goalService.isTurnBuffered()) throw new Error("Pending Goal state is still locked; completion was not applied.");
		}
		const finalEvidenceBlock = evidenceBlockNow();
		if (finalEvidenceBlock) throw new Error(finalEvidenceBlock);
		completeResult = core.goalService.apply(ctx, {
			reconcile: false,
			focusToken: opts.completionFocus,
			mutate: (current) => {
				const target = evidenceKey === null ? opts.goal : { ...current, objective: opts.goal.objective };
				const block = currentSettings.disableTasks || currentSettings.disableContracts ? null : goalEvidenceBlockReason(target, ctx.cwd);
				if (block) throw new Error(block);
				return { ...target, status: "complete" as const, stopReason: "agent" as const, updatedAt: nowIso() };
			},
		});
	} catch (err) {
