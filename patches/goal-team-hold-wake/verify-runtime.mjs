...
const { createJiti } = await import(pathToFileURL(path.join(agentRoot, "npm/node_modules/jiti/lib/jiti.mjs")).href);
...
const agentModules = path.join(agentRoot, "npm/node_modules");
...
await sleep(10);
assert.equal(entries.at(-1).data.state, "bound", "completion without terminal state released the hold");
cases++;
bus.emit(SUBAGENT_ASYNC_COMPLETE_EVENT, {
  runId: run1.runId,
  toolCallId: "call-1",
...
await sleep(10);
assert.equal(entries.at(-1).data.state, "bound", "terminal event released an active native status");
cases++;
...
assert.equal(nativeMessages.length, 1, "native notifier did not produce exactly one wake");
cases++;
bus.emit(SUBAGENT_ASYNC_COMPLETE_EVENT, completed);
await sleep(10);
assert.equal(
...
assert.equal(nativeMessages.length, 1, "duplicate observer event sent a second native wake");
cases++;
...
assert.equal(nativeMessages.length, 2, "native failure did not produce exactly one wake");
