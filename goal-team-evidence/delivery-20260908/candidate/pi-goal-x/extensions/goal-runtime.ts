...
export type GoalContinuationHold = (ctx: ExtensionContext, goal: GoalRecord) => boolean;
...
export class GoalRuntime {
	...
	private continuationHold: GoalContinuationHold | null = null;
	private reconcileContinuationHold: (() => void) | null = null;
	...
	/** Install one parent-runtime hold predicate. It can suppress checkpoints only. */
	setContinuationHold(hold: GoalContinuationHold | null, reconcile: (() => void) | null = null): void {
		this.continuationHold = hold;
		this.reconcileContinuationHold = reconcile;
	}

	private isContinuationHeld(ctx: ExtensionContext, goal: GoalRecord): boolean {
		try {
			return this.continuationHold?.(ctx, goal) === true;
		} catch {
			// A broken optional bridge must not strand an active goal.
			return false;
		}
	}
	...
	clearContinuationState(resetNetworkErrorBackoff = true): void {
		...
		try {
			this.reconcileContinuationHold?.();
		} catch {
			// The optional bridge is fail-open and may not disrupt goal lifecycle.
		}
	}
	...
	queueContinuation(ctx: ExtensionContext, goal: GoalRecord, force = false): void {
		...
		if (this.isContinuationHeld(ctx, goal)) {
			this.clearContinuationTimer();
	...
	private sendQueuedContinuation(ctx: ExtensionContext, scheduledGoalId: string): void {
		...
		this.continuationScheduledFor = null;
		const currentGoal = this.hooks.getGoal();
		if (currentGoal?.id === scheduledGoalId && this.isContinuationHeld(ctx, currentGoal)) {
