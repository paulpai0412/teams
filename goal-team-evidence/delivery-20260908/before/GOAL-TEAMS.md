...
| requestDigest、requestRef | CLI 產生的 SHA-256 correlation 與 raw packet realpath；不由模型手算 |
...
- Goal/task completion 現在檢查目前 branch 的對應 native bindings，阻擋 active／unknown
  ...
  unknown-release 不是 terminal proof。這不是 REQUIRED product evidence／source
  freshness gate，也不能看見其他 session 或未綁定的工作；這些仍由 main/host 驗收。
