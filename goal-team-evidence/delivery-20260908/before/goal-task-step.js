...
if (!/^[a-f0-9]{64}$/.test(input.requestDigest ?? "") || typeof input.requestRef !== "string" || !input.requestRef.startsWith("/")) {
  throw new Error("Prepare the saved packet with parent-only goal-request.mjs before dispatch");
}
...
const identity = {version: 2, requestDigest: input.requestDigest, requestRef: input.requestRef};
...
const record = {...identity, status: passed ? "reported" : "blocked", runId: result?.runId ?? null,
  outputReference: result?.outputReference ?? result?.outputPathMapping?.savedPath ?? null,
...
return {status: record.status, key, record, report: report ?? null,
  artifactPaths: result?.artifactPaths ?? [], outputReference: record.outputReference,
