...
const { loadSettingsSnapshot, loadGoalSettingsFileConfig } = await jiti.import(
...
// Installed completion code reads the PROJECT layer directly; global defaults
// alone are insufficient. Fail closed on a newly onboarded project until set.
assert.equal(
  loadGoalSettingsFileConfig(cwd).disabled,
  true,
  "project must explicitly disable the built-in auditor",
);
...
function fixture(overrides = {}) {
  return {
    teamGoalRequest: { ...request, ...overrides },
...
assert.equal(noSave.data.teamGoalActiveStep, recordKey);
cases++;
