...
assert.deepEqual(receipt.roles.map((r) => r.name).sort(), ['team.debugger', 'team.reviewer']);
...
if (all.status === 0) {
  const full = JSON.parse(all.stdout);
...
checks++;
console.log(JSON.stringify({ status: 'PASS', checks, cwd, modelCalls: 0, childAgents: 0,
  allRoleHealth: all.status === 0 ? 'PASS' : 'FAIL (missing skill preserved)',
  limitation: 'Offline preflight behavior only; not live dispatch or product acceptance.' }, null, 2));
