...
const selectedRoles = roleArgs.length ? roleArgs[0].slice("--roles=".length).split(",") : null;
assert.ok(!selectedRoles || (selectedRoles.every((name) => /^team\.[a-z]+$/.test(name)) && new Set(selectedRoles).size === selectedRoles.length), "--roles requires unique canonical team.* names");
for (const arg of args) {
  assert.ok(!arg.startsWith("-") || ["--models-only", "--roles-only"].includes(arg) || arg.startsWith("--roles="), "unknown option: " + arg);
}
...
if (selectedRoles) {
  for (const name of selectedRoles) {
    ...
    assert.equal(discovered.agents.filter((agent) => agent.name === name).length, 1, "selected role missing or ambiguous: " + name);
  }
} else {
  assert.deepEqual(
...
const agentsToCheck = discovered.agents.filter((agent) => !selectedRoles || selectedRoles.includes(agent.name));
const rows = [];
for (const agent of agentsToCheck) {
  ...
  const scopes = resolveModelScopesForAgent(discovered.modelScope, agent.name, {
  ...
  for (const tool of [
    ...
    assert.ok(!agent.tools.includes(tool), agent.name + " unexpected " + tool);
...
if (rolesOnly) {
  console.log(
    JSON.stringify(
      {
        status: "PASS",
        scope: selectedRoles ? "selected-role-contracts-only" : "role-contracts-only",
        requestedRoles: selectedRoles,
        uncheckedRoles: discovered.agents.filter((agent) => !agentsToCheck.includes(agent)).map((agent) => agent.name),
        modelCalls: 0,
        childAgents: 0,
        cwd,
        roles: rows,
