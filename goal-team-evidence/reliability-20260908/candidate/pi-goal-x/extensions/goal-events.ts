...
export function registerGoalEvents(core: GoalCore): void {
	...
	const currentSessionId = (): string | null => {
		try {
			return currentContext?.sessionManager.getSessionFile()
	...
	const teamHold = new GoalTeamHold({
		sessionId: currentSessionId,
		cwd: () => currentContext?.cwd ?? "",
		goal: () => core.state.goal,
		append: (entry) => pi.appendEntry(GOAL_TEAM_HOLD_ENTRY, entry),
		readStatus: inspectGoalTeamRunStatus,
		recoverStatus: (binding) => recoverGoalTeamRunStatus(pi.events, binding),
		helperPath: join(homedir(), ".pi", "agent", "teams", "goal-task-step.js"),
	});
	const reconcileTeamHold = (): void => {
		const goal = core.state.goal;
		const cwd = currentContext?.cwd;
		if (goal && cwd) teamHold.shouldHold({ cwd }, goal);
		else teamHold.clear("no-focused-goal");
	};
	core.runtime.setContinuationHold(
		(ctx, goal) => teamHold.shouldHold(ctx, goal),
		reconcileTeamHold,
	);
	const unsubscribeTeamCompletion = pi.events.on(SUBAGENT_ASYNC_COMPLETE_EVENT, (event) => {
		void teamHold.onNativeCompletion(event);
	});
	...
	pi.on("tool_result", async (event, ctx) => {
		currentContext = ctx;
		await teamHold.completeToolResult(event);
		if (event.toolName === "subagent" && asRecord(event.input)?.action === "status") {
			const observed = await teamHold.reconcile();
			if (observed?.kind === "unknown") {
				return { content: [...event.content, { type: "text", text: `Team run ${observed.runId}: reconciliation required; status is unknown. Inspect retained mission intent and native receipt before any redispatch or completion.` }] };
			}
		}
	});
	...
	pi.on("session_start", async (event, ctx) => {
		...
		await teamHold.recover(ctx.sessionManager.getBranch());
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		if (incomingGoalId !== null) {
			...
			if (!core.isActionableContinuationGoal(incomingGoalId)) {
				try {
					...
					// Best-effort abort; the stale-checkpoint tool guard and prompt still apply.
				}
				core.updateUI(ctx);
				return {
		...
		// Recheck once at a parent-run boundary, including a missed completion event.
		// No timer, child launch, mission mutation or second wake path is introduced.
		currentContext = ctx;
		const teamObservation = await teamHold.reconcile();
		...
		if (teamObservation?.kind === "unknown") {
			prompt += `\n\n[TEAM RUN RECONCILIATION REQUIRED runId=${teamObservation.runId}]\nNative status is unknown. Inspect preserved mission intent and exact native receipts; do not redispatch or complete the task from this observation.`;
		}
	...
	pi.on("session_shutdown", async (_event, ctx) => {
		...
		unsubscribeTeamCompletion?.();
		core.runtime.setContinuationHold(null);
		teamHold.dispose();
		if (core.state.goal) core.persist(ctx);
	});
}
