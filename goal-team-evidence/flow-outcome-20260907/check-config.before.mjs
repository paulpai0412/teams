...
const cwdArg = process.argv
  .slice(2)
  .find((arg) => !["--models-only", "--roles-only"].includes(arg));
...
if (rolesOnly) {
  console.log(
    JSON.stringify(
      {
        status: "PASS",
        scope: "role-contracts-only",
        modelCalls: 0,
        childAgents: 0,
        cwd,
        roles: rows,
        limitations: [
          "Checks role discovery, models, skills, MCP selectors and permissions; not global extension loading, live child execution or OS isolation.",
        ],
      },
      null,
      2,
