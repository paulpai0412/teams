...
if (!Number.isInteger(input.attempt) || input.attempt < 1 || input.attempt > 4)
  throw new Error("Attempt must be 1..4");
if (
...
// Same task + role shares a repair ceiling, not a successful-work ceiling.
...
const budget = (await state.get(budgetKey)) ?? { version: 2, repairs: 0 };
if (budget.version !== 2 || !Number.isInteger(budget.repairs) || budget.repairs < 0 || budget.repairs > 3)
  throw new Error('Legacy/invalid admission accounting requires parent reconciliation');
let repairs = budget.repairs;
...
result = await runs.run(key, {
  ...
  // A throw proves neither that no child ran nor that no report was captured.
  await state.set(recordKey, {
    ...identity, status: 'blocked',
    outcomes: { execution: 'unknown', reportDelivery: 'unknown', product: 'unknown' },
    error: String(error?.message ?? error).slice(0, 2048),
    ...(typeof error?.runId === 'string' ? { runId: error.runId.slice(0, 256) } : {}),
    ...(typeof error?.outputReference === 'string' ? { outputReference: error.outputReference.slice(0, 2048) } : {}),
    ...(typeof error?.structuredOutputPath === 'string' ? { structuredOutputPath: error.structuredOutputPath.slice(0, 2048) } : {}),
  });
  throw error;
}
