...
/** Native execution gate only; terminal state never proves product acceptance. */
export async function goalTeamRunBlockReason(entries: readonly unknown[], goalId: string, cwd: string, taskId?: string): Promise<string | null> {
	...
	for (const entry of latest.values()) {
		// These entries are written by the host only after exact terminal status
		// validation. They survive native artifact cleanup, not an unknown release.
		if (entry.state === "released" && (entry.reason === "native-completion" || entry.reason === "reconcile-terminal")) continue;
		const observed = await inspectGoalTeamRunStatus(entry.binding);
		if (observed.kind !== "terminal" || observed.completionOwnerId !== entry.binding.completionOwnerId) {
			return `Team run ${entry.binding.runId} is ${observed.kind === "active" ? "still active" : "unknown"}; inspect native status and retained mission intent before completion.`;
		}
	}
	return null;
}
...
export class GoalTeamHold {
	...
	/** One parent-boundary observation, not a timer or permission to redispatch. */
	async reconcile(): Promise<GoalTeamRunObservation | null> {
		const binding = this.binding;
		if (!binding) return null;
		if (!currentMatches(binding, this.deps.goal(), this.deps.sessionId(), this.deps.cwd())) {
			this.clear("reconcile-context-mismatch");
			return null;
		}
		let observed: GoalTeamRunObservation;
		try {
		...
		// Context/ownership may change while the observation is in flight.
		if (!this.binding || !sameBinding(this.binding, binding)) return null;
		if (!currentMatches(binding, this.deps.goal(), this.deps.sessionId(), this.deps.cwd())) {
			this.clear("reconcile-context-mismatch");
			return null;
		}
		if (observed.completionOwnerId !== binding.completionOwnerId
			|| observed.runId !== binding.runId || observed.sessionId !== binding.sessionId
			|| observed.cwd !== binding.cwd || observed.toolCallId !== binding.toolCallId) {
			observed = { kind: "unknown" };
		}
		if (observed.kind !== "active") {
			this.release(binding, observed.kind === "terminal" ? "reconcile-terminal" : "reconcile-required-unknown");
		}
		// Unknown releases checkpoint suppression only; mission intent stays owned.
		return { ...observed, runId: binding.runId };
	}
