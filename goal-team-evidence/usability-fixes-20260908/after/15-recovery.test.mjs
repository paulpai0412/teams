...
test('captured pass cannot receive retry permission; report-only never authorizes replay',async()=>{const f=fixture();try{
 ...
 f.state['goal-admissions.task.team.reviewer']={version:2,repairs:0,lastRecord:f.recordKey};
 await assert.rejects(execute(state,{run:async()=>{throw new Error('must not launch')}}),/recovery required/);
 }finally{fs.rmSync(f.cwd,{recursive:true,force:true});}});
