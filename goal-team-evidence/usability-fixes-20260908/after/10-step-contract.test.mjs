...
test('renaming phases cannot reset three authorized repairs after initial failure',async()=>{
 ...
 for(let n=1;n<=4;n++){
   ...
   f.data['goal-step.t.renamed-'+n+'.1'].recovery={action:'retry',sourceState:input.sourceState,reason:'fixture diagnosed repair',nativeStatusRef:'/fixture/status.json',hostReceiptRef:'/fixture/check.json',nativeDigest:'a'.repeat(64),hostReceiptDigest:'b'.repeat(64),childRunId:'child-'+n};
