...
同 task/role 最多三次 admission，換 phase 不重置；後續 admission 需 reason/evidence。
...
`patches/goal-team-delivery/` 是 reliability v2 之上的增量；套用後用其 `apply.mjs --check/--verify`，不要拿舊增量的重疊 post hashes 判斷最新安裝。REQUIRED review/live canary 不因 offline fixture PASS 而免除。
...
```bash
node ~/.pi/agent/teams/goal-request.mjs /absolute/evidence/packet.json > /absolute/evidence/prepared.json
```

將 prepared JSON **原樣**交 `state.set("teamGoalRequest", prepared)`；不可改 task/source
...
| retry | 後續同 task/role admission 必須 `{reason,evidence}`；不作為自動 retry 或繞過 ceiling 的授權 |
