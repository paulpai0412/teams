...
test('renaming phases cannot reset three authorized repairs after initial failure',async()=>{
 const f=fixture(); let launches=0;
 const runs={run:async()=>{launches++;return {ok:true,runId:'child-'+launches,structuredOutput:report({verdict:'failed'})};}};
 for(let n=1;n<=4;n++){
   ...
   assert.equal((await runStep(f.state,runs)).status,'blocked');
   // Consumer fixture only; recovery.test.mjs verifies native+host preparation.
   f.data['goal-step.t.renamed-'+n+'.1'].recovery={action:'retry',requestDigest:f.data['goal-step.t.renamed-'+n+'.1'].requestDigest,sourceState:input.sourceState,reason:'fixture diagnosed repair',nativeStatusRef:'/fixture/status.json',hostReceiptRef:'/fixture/check.json',nativeDigest:'a'.repeat(64),hostReceiptDigest:'b'.repeat(64),childRunId:'child-'+n};
 ...
 await assert.rejects(runStep(f.state,runs),/repair limit/);
...
test('missing report cannot trigger another same-source launch after clearing active marker',async()=>{
 ...
 await assert.rejects(runStep(f.state,runs),/recovery required/);
 assert.equal(launches,1);
});
test('normal successful progression does not require pretending a failure occurred',async()=>{
 ...
 // Consumer fixture; real terminal/host preparation is covered by reconciliation.test.mjs.
 const previous=f.data['goal-step.t.review.1'];
 previous.recovery={action:'continue',requestDigest:previous.requestDigest,sourceState:input.sourceState,reason:'fixture parent accepted progress',nativeStatusRef:'/fixture/status.json',hostReceiptRef:'/fixture/check.json',nativeDigest:'a'.repeat(64),hostReceiptDigest:'b'.repeat(64),childRunId:'child'};
