...
test('four successful new candidate slices need no retry permission',async()=>{
 ...
 for(let n=1;n<=4;n++){
  packet(f,n,{sourceState:'sha256:'+n});assert.equal((await execute(f.state,runs)).status,'reported');
  // Consumer fixture; reconciliation.test.mjs exercises the real producer.
  const record=f.data['goal-step.task.slice-'+n+'.1'];
  record.recovery={action:'continue',requestDigest:record.requestDigest,sourceState:'sha256:'+(n+1),reason:'fixture parent accepted progress',nativeStatusRef:'/fixture/status.json',hostReceiptRef:'/fixture/check.json',nativeDigest:'a'.repeat(64),hostReceiptDigest:'b'.repeat(64),childRunId:'child'};
